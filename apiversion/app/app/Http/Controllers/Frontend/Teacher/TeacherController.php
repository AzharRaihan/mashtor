...
class TeacherController extends Controller
{
    ...
    public function tutorDetails($id){
        $data = [];
        $data['tutors'] = DB::table('users')
        ->select('users.id as user_id','users.fullname','users.image','users.become_a_tutor','user_details.id as user_details_id','user_details.profile_tag','user_details.price','user_details.user_description')
        ->join('user_details','user_details.user_id','=','users.id')
        ->where('user_id',$id)
        ->where('users.become_a_tutor',1)
        ->first();
